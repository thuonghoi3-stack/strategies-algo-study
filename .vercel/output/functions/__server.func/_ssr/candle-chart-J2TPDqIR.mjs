...
//#region node_modules/.nitro/vite/services/ssr/assets/candle-chart-J2TPDqIR.js
...
function CandleChart({ bars, result, height = 440, className }) {
	...
	(0, import_react.useEffect)(() => {
		...
		const draw = (hoverI, mx, my) => {
			...
			const slot = plotW / n;
			const bodyW = Math.max(1.2, Math.min(9, slot * .62));
			let lo = Infinity;
			let hi = -Infinity;
			const consider = (v) => {
			...
			lo -= pad;
			hi += pad;
			const yPrice = (v) => priceTop + (hi - v) / (hi - lo) * priceH;
			const xAt = (i) => padL + (i + .5) * slot;
			...
			for (const ov of overlays) if (ov.kind === "levels") for (const lv of ov.levels) {
			...
			for (const ov of overlays) {
				...
				if (ov.kind === "dots") for (let i = 0; i < ov.values.length; i++) {
					const v = ov.values[i];
					if (v == null) continue;
			...
			result?.oscillators.slice(0, 2).forEach((osc, oi) => {
				...
				let omin = Infinity;
				let omax = -Infinity;
				for (const s of osc.series) for (const v of s.values) {
				...
				if (!Number.isFinite(omin) || !Number.isFinite(omax) || omax === omin) {
					...
					const padO = (omax - omin) * .12 || 1;
					omin -= padO;
					omax += padO;
				}
				const yO = (v) => top + (omax - v) / (omax - omin) * oscH;
				ctx.strokeStyle = border;
				ctx.beginPath();
				ctx.moveTo(padL, top);
				...
				for (const s of osc.series) if (s.type === "hist") for (let i = 0; i < s.values.length; i++) {
					const v = s.values[i];
					if (v == null) continue;
				...
				else drawLine(ctx, s.values, s.color, xAt, yO, 1.3, false);
				ctx.fillStyle = muted;
				ctx.textAlign = "left";
		...
		const onMove = (e) => {
			...
			const padL = 12;
			const plotW = rect.width - padL - 56;
	...
	const chg = active && hover && hover.i > 0 ? active.c - (result?.chartBars ?? bars)[hover.i - 1].c : null;
...
function drawLine(ctx, values, color, xAt, yAt, width, dashed) {
...
function drawBand(ctx, ov, xAt, yAt, n) {
	ctx.beginPath();
	let started = false;
	for (let i = 0; i < n; i++) {
		const u = ov.upper[i];
...
function drawCloud(ctx, ov, xAt, yAt, n) {
	for (let i = 1; i < n; i++) {
		const a0 = ov.a[i - 1];
		const a1 = ov.a[i];
		const b0 = ov.b[i - 1];
